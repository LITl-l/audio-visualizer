...
// import ShaderCanvas from '@/components/ShaderCanvas'; // Original static import
import YouTubePlayerFactory from "youtube-player";
import type { YouTubePlayer } from "youtube-player/dist/types";
import dynamic from "next/dynamic"; // Added for dynamic import

// Dynamically import ShaderCanvas with SSR disabled
const ShaderCanvas = dynamic(() => import("@/components/ShaderCanvas"), {
  ssr: false,
  loading: () => (
    <div className="absolute top-0 left-0 w-full h-full -z-10 bg-gray-900 flex items-center justify-center">
...
export default function HomePage() {
  ...
  useEffect(() => {
    if (playerContainerRef.current && !ytPlayerInstanceRef.current) {
      ...
      ytPlayerInstanceRef.current = YouTubePlayerFactory(
      ...
      ytPlayerInstanceRef.current.on(
        "stateChange",
        (event: { data: number }) => {
          if (event.data === 1) {
            // Playing
  ...
  return (
    ...
    <main className="relative w-screen h-screen flex flex-col items-center justify-start overflow-hidden">
      ...
      <div className="z-10 flex flex-col items-center justify-start w-full px-4 pt-12 sm:pt-20">
        {" "}
        {/* Increased padding-top */}
        <div className="w-full max-w-lg md:max-w-xl">
          {" "}
          {/* Constrain width of input area */}
